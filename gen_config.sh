...
EOF
    ...
    echo -e "${YELLOW}⚠️  未找到本机配置，跳过。${PLAIN}"
fi
...
if [ -f "$MANUAL_NODES_FILE" ] && [ -s "$MANUAL_NODES_FILE" ]; then
    ...
    [[ $TOTAL_COUNT -eq 0 ]] && TOTAL_COUNT=$(grep -cve '^\s*$' "$MANUAL_NODES_FILE")

    read -p "$(echo -e "${YELLOW}❓ 发现 ${BOLD}${TOTAL_COUNT}${PLAIN}${YELLOW} 个旧节点，保留？(n=清空) [y/n]: ${PLAIN}")" keep_manual
    if [[ "$keep_manual" == "n" || "$keep_manual" == "N" ]]; then
        echo "" > "$MANUAL_NODES_FILE"
        echo -e "${BLUE}   🗑️  已清空${PLAIN}"
    else
        echo -e "${GREEN}   ✔  已保留${PLAIN}"
    fi
else
    touch "$MANUAL_NODES_FILE"
fi

read -p "$(echo -e "${YELLOW}❓ 是否添加新链接？[y/n]: ${PLAIN}")" add_manual
...
read -p "$(echo -e "${YELLOW}❓ 打印文件内容？[y/n]: ${PLAIN}")" print_content
